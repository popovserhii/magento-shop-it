...
class Amasty_Shopby_Model_Url_Builder
{
    ...
    public function reset()
    {
        ...
        $this->brandUrlKey = Mage::getStoreConfig('amshopby/brands/url_key');
    ...
    protected function getBasePart($paramPart)
    {
        ...
        elseif ($this->getCategoryId() == $rootId) {
            ...
            if($this->isBrandPage() && $this->brandUrlKey && ($this->mode!= Amasty_Shopby_Model_Source_Url_Mode::MODE_DISABLED)){
                $url .= $this->brandUrlKey;
                if ($seoAttributePartExist) {
                    $url .=  '/';
                }
            }
